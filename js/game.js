(function(){
  ...
  function resizeCanvas(){
    ...
    if(rect.width===0 || rect.height===0) return; // wait until layout
  ...
  // keyboard handlers with normalization
  ...
  // dynamic sizes based on canvas (logical pixels after transform)
  ...
  function draw(){
    const W=getW(), H=getH();
    // clear
    ctx.clearRect(0,0,W,Math.floor(H));
    // draw road
    const laneWidth=W*0.6; const laneX=(W-laneWidth)/2;
    ...
    // draw player (with slight tilt when moving)
    ctx.save();
  ...
  function setupTouchControls(){
    ...
    if(leftBtn && rightBtn){
      ...
      // fallback: pointer zones on canvas
